    ...
    <div class="auth-wrapper">
      <div class="auth-brand" ref="brandRef">
        ...
        <div class="brand-terminal">
          <div class="t-bar"><div class="t-dots"><span></span><span></span><span></span></div><span class="t-title">~/register — zsh</span></div>
          <div class="t-body">
            <div class="t-line"><span class="prompt">$</span> <span class="cmd">echo "Create your account"</span></div>
            <div class="t-line"><span class="out">Create your account</span></div>
            <div class="t-line t-spacer"></div>
            <div class="t-line"><span class="prompt">$</span> <span class="cmd">join mentalblog.community</span></div>
            <div class="t-line"><span class="out">Connecting... [OK]</span></div>
            <div class="t-line t-spacer"></div>
            <div class="t-line"><span class="prompt">$</span> <span class="cmd">cat welcome.txt</span></div>
            <div class="t-line"><span class="out">Join us, share insights</span></div>
            <div class="t-line t-spacer"></div>
      ...
      <div class="auth-form" ref="formRef">
        <div class="af-container">
          ...
          <form @submit.prevent="handleRegister" ref="formInnerRef">
            <div class="field"><label>Username</label><input v-model="form.username" class="input-cyber" placeholder="3-50 characters" required /><span v-if="errors.username" class="field-err">{{ errors.username }}</span></div>
            <div class="field"><label>Email</label><input v-model="form.email" type="email" class="input-cyber" placeholder="your@email.com" required /><span v-if="errors.email" class="field-err">{{ errors.email }}</span></div>
            <div class="field"><label>Password</label><input v-model="form.password" type="password" class="input-cyber" placeholder="At least 6 characters" required minlength="6" /><span v-if="errors.password" class="field-err">{{ errors.password }}</span></div>
            <div class="field"><label>Nickname</label><input v-model="form.nickname" class="input-cyber" placeholder="Pick a name" /><span v-if="errors.nickname" class="field-err">{{ errors.nickname }}</span></div>
            <button type="submit" class="btn-cyber-full" :disabled="loading">{{ loading ? 'Registering...' : 'Register' }}</button>
          </form>
          <p class="af-footer">Already have an account? <router-link to="/login">Sign in</router-link></p>
        </div>
      </div>
    </div>
  </div>
...
const errors = reactive({ username: '', email: '', password: '', nickname: '' })

function validate(): boolean {
  errors.username = ''; errors.email = ''; errors.password = ''; errors.nickname = ''
  ...
  return !errors.username && !errors.email && !errors.password && !errors.nickname
}
...
async function handleRegister() {
  ...
  try {
    ...
    router.push('/login')
  } catch (e: any) { ElMessage.error(e.message || 'Registration failed') }
  loading.value = false
}
</script>
<style scoped>
...
.auth-wrapper { display: flex; width: 100%; max-width: 800px; border-radius: 12px; overflow: hidden; border: 1px solid rgba(0,240,255,0.08); min-height: 520px; }
...
.t-dots span:nth-child(1) { background: #FF5F57; } .t-dots span:nth-child(2) { background: #FEBC2E; } .t-dots span:nth-child(3) { background: #28C840; }
