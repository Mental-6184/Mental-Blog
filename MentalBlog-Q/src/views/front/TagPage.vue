<template>
  <div class="tag-pg" v-if="pageReady">
    <div class="container">
      <div class="page-h" ref="headerRef"><h1>#{{ tagName||'Tag' }}</h1><p>{{ articles.length }} articles</p></div>
      <div class="cw">
        <div class="main-col">
          <div v-if="loading" class="empty-state"><p>Loading...</p></div>
          <div v-else-if="articles.length===0" class="empty-state"><p>No articles with this tag</p></div>
        ...
        <Sidebar context="tag" :context-name="tagName" :context-slug="route.params.slug" :hot-articles="articles.slice(0,5)" />
      </div>
    </div>
  </div>
</template>
...
const articles = ref<any[]>([]); const tagName = ref('')
...
async function loadData() {
  ...
  try {
    const tr = await getTags(); const tags = (tr as any).data || []
    const tag = tags.find((t: any) => t.slug === slug)
    if (tag) { tagName.value = tag.name; const r = await getArticles({ page: 1, pageSize: 100, tagId: tag.id }); articles.value = (r as any).records || [] }
    else { tagName.value = slug; articles.value = [] }
  } catch (e) { tagName.value = slug }
...
<style scoped>
.tag-pg { background: var(--bg-deep); min-height: 60vh; padding: 44px 0 100px; }
.page-h { margin-bottom: 28px; }
.page-h h1 { font-size: 26px; font-weight: 800; color: var(--cyber-cyan); letter-spacing: -.5px; margin-bottom: 2px; font-family: var(--font-mono); }
